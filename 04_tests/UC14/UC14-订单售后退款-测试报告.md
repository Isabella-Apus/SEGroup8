...
| 编号 | 层级 | 场景 | 关键断言 |
|---|---|---|---|
| `INT-UC14-001` | MySQL Integration | 买家申请、卖家拒绝、再次申请、管理员同意 | 状态正确；日志动作、处理人、角色、理由和顺序完整 |
| `INT-UC14-002` | MySQL Integration | 管理员拒绝 | 订单保持原业务状态；无退款流水 |
| `INT-UC14-003` | MySQL Integration | 待发货仅退款自动通过 | 买家到账；订单关闭；新品库存仅回补一次 |
| `INT-UC14-004` | MySQL Integration | 仅退款/退货退款边界 | 非法模式及运输中退货退款拒绝且数据不变；退货退款不自动回补库存 |
| `INT-UC14-005` | MySQL + HTTP | 买家、卖家、管理员权限 | 非本人买家、非商品卖家、非管理员均拒绝且无资金副作用 |
| `INT-UC14-006` | MySQL Integration | 已结算订单退款 | 买家回流、卖家扣回；两条退款流水金额净和为零 |
| `INT-UC14-007` | MySQL Integration | 卖家/管理员并发同意 | 仅一方成功；仅一笔退款和一条同意日志 |
| `WEB-UC14-001/002` | WebMvc | 管理端参数和日志查询合约 | 非法参数拒绝；日志响应字段完整 |
| `E2E-UC14-001/002` | Playwright | 买家申请到卖家处理、管理员仲裁 | 刷新后售后状态和处理记录仍持久化 |
