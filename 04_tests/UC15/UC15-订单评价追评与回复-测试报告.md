...
| `API-UC15-001` | MySQL Integration | 首评、回复、追评闭环 | 仅实际卖家可回复；追评以首评为前置且最多一次 |
| `API-UC15-002` | MySQL Integration | 多商品中途写入失败 | 第一条已写入后第二条触发数据库异常，整个事务回滚且订单仍待评价 |
| `API-UC15-003` | WebMvc | 参数和卖家空集合 | 响应结构正确，无商品卖家不会执行无约束评价查询 |
| `API-UC15-004` | MySQL Integration | 买家/卖家分页隔离 | `total` 与分页记录一致，不返回其他买家或其他卖家的评价 |
| `E2E-UC15-001` | Playwright | 买家首评、卖家回复、买家追评 | 刷新或重新查询后评价和订单状态仍持久化 |
