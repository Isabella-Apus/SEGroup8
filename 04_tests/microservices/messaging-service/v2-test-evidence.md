...
| `mvn -B -f backend/pom.xml test` (V3 final regression) | PASS | 235 | 0 | 0 | Docker/Testcontainers available; all backend tests pass. Shutdown-only scheduler connection noise is environment-dependent and does not affect the Maven result |
...
- Scenario C is closed by the live stop/restart record in `evidence/raw-reports/scenario-c-live.md`: the same payment event moved from producer retry to published, Inbox PROCESSED, and one persisted notification after Messaging restart.
