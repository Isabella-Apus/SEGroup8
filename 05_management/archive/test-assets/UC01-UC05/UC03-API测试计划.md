...
| INT03-03 | `MerchantApplicationControllerWebMvcTest.reject_shouldRequireReason` | Controller API | 拒绝原因参数校验 |
| INT03-04 | `MerchantApplicationControllerWebMvcTest.approve_shouldReturnSuccessAndRecordAudit` | Controller API | 管理员通过和审计调用 |
...
- 审核通过必须验证 Controller 调用了审计服务；角色/店铺/通知的细节由 Service 测试验证。
- 完整管理员 JWT、事务回滚和前端 E2E 留到后续回归。
