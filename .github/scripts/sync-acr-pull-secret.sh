...
cat <<EOF | ssh \
  ...
  "test -r \"\$HOME/.kube/config\"; export KUBECONFIG=\"\$HOME/.kube/config\"; kubectl --namespace '$K8S_NAMESPACE' apply -f - >/dev/null"
apiVersion: v1
kind: Secret
metadata:
...
ssh \
  ...
  -o ServerAliveCountMax=5 \
  "$DEPLOY_USER@$DEPLOY_HOST" \
  "test -r \"\$HOME/.kube/config\"; export KUBECONFIG=\"\$HOME/.kube/config\"; kubectl --namespace '$K8S_NAMESPACE' get secret acr-pull-secret -o jsonpath='{.data.\\.dockerconfigjson}' | base64 -d | grep -Fq '\"$ACR_REGISTRY\"'"
