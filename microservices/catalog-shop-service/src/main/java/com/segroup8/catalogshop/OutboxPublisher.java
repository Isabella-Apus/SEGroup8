...
class OutboxPublisher {
    ...
    private void publish(OutboxRow event){String base="MESSAGING".equals(event.destination())?messagingUrl:orderUrl;if(base==null||base.isBlank())return;String trace=event.eventId();try{MDC.put("eventId",event.eventId());Map<String,Object> payload=json.readValue(event.payload(),new TypeReference<>(){});Map<String,Object> envelope=new LinkedHashMap<>();envelope.put("eventId",event.eventId());envelope.put("eventType",event.eventType());envelope.put("eventVersion",1);envelope.put("producer","catalog-shop-service");envelope.put("aggregateType",event.aggregateType());envelope.put("aggregateId",event.aggregateId());envelope.put("occurredAt",event.createdAt());envelope.put("traceId",trace);envelope.put("payload",payload);client.post().uri(base+"/internal/events").contentType(MediaType.APPLICATION_JSON).header("X-Internal-Service-Token",internalToken).header("X-Request-Id",trace).header("X-Trace-Id",trace).header("Idempotency-Key",event.eventId()).header("X-Idempotency-Key",event.eventId()).body(envelope).retrieve().toBodilessEntity();db.sql("update outbox_event set status='SENT',sent_at=CURRENT_TIMESTAMP,last_error=null where id=:id and status='PENDING'").param("id",event.id()).update();}catch(Exception failure){long seconds=Math.min(300,1L<<Math.min(event.attempts()+1,8));String message=failure.getMessage()==null?failure.getClass().getSimpleName():failure.getMessage();if(message.length()>500)message=message.substring(0,500);db.sql("update outbox_event set attempts=attempts+1,status=case when attempts+1>=:max then 'DEAD' else 'PENDING' end,next_attempt_at=TIMESTAMPADD(SECOND,:seconds,CURRENT_TIMESTAMP),last_error=:error where id=:id and status='PENDING'").params(Map.of("max",MAX_ATTEMPTS,"seconds",seconds,"error",message,"id",event.id())).update();}finally{MDC.remove("eventId");}}
}
