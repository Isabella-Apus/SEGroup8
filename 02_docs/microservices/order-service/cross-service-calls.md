...
二手入站建单额外强校验 `Idempotency-Key=tradeType:tradeId`。请求头与请求体业务键不一致时返回 `IDEMPOTENCY_KEY_MISMATCH`，防止调用方用一个幂等键创建另一笔交易；随后可用同一业务键查询 `/internal/orders/by-business-key/{key}` 恢复不确定结果。
