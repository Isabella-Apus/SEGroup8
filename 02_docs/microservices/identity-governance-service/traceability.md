...
| REQ02 / UC02 | `UserController`、地址事务 | `user`、`address` | `PublicApiSuccessCoverageTest`、`IdentityGovernanceFlowIntegrationTest#profileAndAddress...`、既有 `uc02-profile-address.spec.ts` | API/集成/E2E PASS |
...
公开接口全量鉴权、越权和失效账户状态矩阵见 `service-api-list.md`；真实 MySQL 的 Controller → JWT → Service → JDBC → Flyway 链路见 `MySqlMigrationIntegrationTest#realMySqlTraversesControllerJwtServiceJdbcAndFlywaySchema`。
