...
| 编号 | 层级 | 场景 | 核心断言 |
|---|---|---|---|
| `INT-TC11-001` | HTTP + DB Integration | 合法商品、本人地址、无券；重复项合并 | 响应、`order_info`、`order_item`、服务端价格和库存一致 |
| `INT-TC11-002` | HTTP + DB Integration | 下架、缺货、地址越权、自购、双向拉黑 | 明确拒绝，订单和库存无副作用 |
| `INT-TC11-003` | HTTP + DB Integration | 空商品和非法数量 | DTO 校验拒绝且不写业务数据 |
| `INT-TC11-004` | HTTP + DB Integration | 合法店铺券 | 折扣、承担额、应付金额和券占用一致 |
| `INT-TC11-005` | HTTP + DB Integration | 未领取、门槛不足、店铺不匹配、不存在的券 | 拒绝并回滚订单、库存和券 |
| `INT-TC11-006` | HTTP + DB Integration | 后续商品失败 | 先前库存修改、订单主从和券全部回滚 |
| `INT-TC11-007` | HTTP + DB Integration | 相同幂等键重复提交 | 响应回放，只建一单且只扣一次库存 |
| `E2E-TC11-001` | Playwright + MySQL | 商品加购、购物车结算、订单详情、刷新 | 创建响应成功，详情与重新查询的数据持久一致 |
